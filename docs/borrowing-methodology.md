...
The simulator report includes a model-audit evidence pack. It collects scenario composition, baseline-vs-scenario PSNB and debt/GDP comparisons, methodology range coverage, live borrowing-regime probabilities, calibration artifact status, historical backtest pass rates, prior sensitivity, uncertainty decomposition, and stated model limitations in one reviewable panel. The panel exposes a compact JSON payload and downloadable Markdown/JSON appendix for external review.
