...
The fan also carries an endogenous policy-reaction branch. The headline breach and headroom bands are still reported before discretionary offsets, so the raw fiscal risk remains visible. Separately, any draw that breaches the rule or leaves very thin headroom selects a stress-contingent offset package. The selector first scores mechanical fiscal conditions: credibility/rate-stress and large annual gaps favour tax-led action, inflationary demand pressure favours spending-led restraint, weak growth raises implementation-delay risk, and normal tight-headroom draws lean balanced. Optional institutional priors then adjust those scores for credibility repair, public-service protection, spending-restraint mandates, tax-lock constraints, or late-parliament timing. The selected package is appended to the original scenario and reprojected through the same GE loop, so its GDP drag, CPI/Bank Rate effects, debt/GDP change, gilt-yield feedback, and borrowing-cost feedback all affect the post-reaction headroom and debt bands. If package caps bind, the residual policy gap remains in the post-reaction branch.
