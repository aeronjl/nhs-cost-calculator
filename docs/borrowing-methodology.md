...
The selector is checked against curated consolidation episodes in `src/data/fiscal-reaction-backtests.ts`. Each episode records the historical fiscal stress, the package actually chosen, and the encoded tax/spending measures from the annotated budget corpus. The audit scores exact package-label matches, tax/spending-share distance, and named-lever overlap. Misses are treated as model information rather than hidden errors: for example, spending-led austerity episodes reveal where the reduced-form rule arithmetic needs explicit political or institutional priors.
